...
public class Graph extends ServiceQuery {
	...
	@ApiParam(value = "BioPAX graph traversal type.", required = true, example = "PATHSBETWEEN")
	private GraphType kind;
	
	@NotEmpty(message="Provide at least one source URI.")
	@ApiParam(value = "Source BioPAX entity URIs or standard identifiers (e.g., gene symbols)",
		required = true, example = "FGFR2")
	private String[] source;

	@ApiParam(value = "Target BioPAX entity URIs or standard identifiers (e.g., gene symbols);" +
		"this parameter works only with kind=PATHSFROMTO graph queries.",
		required = false, example = "FGFR2")
	private String[] target;
	
	@Min(1) //note: this allows for null
	@ApiParam(value = "Graph search distance limit", required = false, defaultValue = "1")
	private Integer limit;

	@ApiParam(value = "Graph search direction.", required = false, defaultValue = "UNDIRECTED",
		example = "BOTHSTREAM")
	private Direction direction;
	
	@NotNull(message="Illegal Output Format")
	@ApiParam(value = "Output format name.", required = false, example = "JSONLD",
		defaultValue = "BIOPAX")
	private OutputFormat format;

	@ApiParam(value = "Filter by organism, e.g., taxonomy ID (recommended) or name.", example = "9606")
	private String[] organism;
	@ApiParam(value = "Filter by data source name, id or uri.", example = "reactome")
	private String[] datasource;

	@ApiParam(value="If format is SIF or TXT, one can specify interaction types to apply", example = "interacts-with")
