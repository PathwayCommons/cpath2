...
public final class PreMerger {
  ...
  void buildWarehouse() {
    ...
    for (Metadata metadata : service.metadata().findAll()) {
      ...
      for (String datafile : metadata.getFiles()) {
        try {
          inputStream = new GZIPInputStream(new FileInputStream(service.normalizedFile(datafile)));
          Model m = new SimpleIOHandler(BioPAXLevel.L3).convertFromOWL(inputStream);
          m.setXmlBase(xmlBase);
          warehouse.merge(m);
        } catch (IOException e) {
          log.error("buildWarehouse(), skip: failed to load " + service.normalizedFile(datafile), e);
        }
      }
    }
                        ...
                        Cleaner cleaner, Converter converter) throws IOException
  {
    ...
    if(cleaner != null) {
      ...
      File outputFile = new File(service.cleanedFile(inputDataFile));
    ...
    if (converter != null) {
      ...
      File outputFile = new File(service.convertedFile(inputDataFile));
      if(outputFile.exists()) {
        log.info("pipeline(), re-use " + outputFile.getName());
      } else {
        try {
    ...
    if(Files.exists(Paths.get(service.normalizedFile(inputDataFile)))) {
  ...
  private void checkAndNormalize(Metadata metadata, File file) throws IOException
  {
    ...
    if(metadata.getType() == METADATA_TYPE.MAPPING) {
      ...
      try {
        ...
        service.saveValidationReport(validation, service.validationFile(filename));
    ...
    // save
    try {
      OutputStream out = new GZIPOutputStream(new FileOutputStream(service.normalizedFile(filename)));
