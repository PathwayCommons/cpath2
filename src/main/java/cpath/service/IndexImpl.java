...
public class IndexImpl implements Index, Mappings {
		...
		try {	
			...
			if(page>0) {
				...
				TopScoreDocCollector collector = TopScoreDocCollector
						.create(maxHitsPerPage*(page+1), maxHitsPerPage*(page+1)); //todo: unsure first param
	...
	public void save(BioPAXElement bpe) {
		...
		//exclude form the index the autogenerated comments (e.g. by normalizer, merger)
		...
		Pattern isoformIdPattern = Pattern.compile(MiriamLink.getDatatype("uniprot isoform").getPattern());
		Pattern uniprotIdPattern = Pattern.compile(MiriamLink.getDatatype("uniprot knowledgebase").getPattern());
...
//			doc.add(new StringField(FIELD_XREFID, id.toLowerCase(), Field.Store.NO)); //todo: why did we do this?.. IDs are case sensitive.
	...
	public void save(Model model) {
		//todo: do we really want to index ERs and Provenance (forgot why, which web queries use)?..
	...
	@Override
	public List<Mapping> findBySrcIdInAndDstDbIgnoreCase(String srcId, String dstDb) {
		return findBySrcIdInAndDstDbIgnoreCase(Collections.singletonList(srcId), dstDb);
	}
