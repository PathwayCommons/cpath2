# cPath2 instance configuration

Directory 'work' is for cPath2 configuration, data files and indices home directory.
It is also where one should start the program from.  
...
Run the cpath2.sh script, where specific system and JVM options are defined, 
to execute a series of commands to create a new cpath2 instance (import metadata, 
clean/convert/normalize input data, build the warehouse, build the main BioPAX model, 
Lucene index, and downloads).

Once a new instance is configured and created, - run the web service using the same 
script as follows:

    nohup sh cpath2.sh 2>&1 &

(And then check the nohup.out and cpath2 logs periodically.)
...
Extras/other steps (optional):
 ...
 - -pack (creates a tarball containing all the data to be uploaded somewhere on a file server)
