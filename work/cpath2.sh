...
if [ "$1" = "server" ] ; then
  ...
  export CPATH2_HOME="."
  $JAVA_HOME/bin/java -server $CPATH2_OPTS $CPATH2_TUNING_OPTS -jar ../target/cpath2.war
elif [ "$1" = "console" ] ; then
  export CPATH2_HOME="."
  $JAVA_HOME/bin/java $CPATH2_OPTS -jar ../target/cpath2.war "$2" "$3" "$4" "$5"
else
  ...
  $JAVA_HOME/bin/java -Dfile.encoding=UTF-8 -Xmx4g -jar ../target/cpath2.war
fi
